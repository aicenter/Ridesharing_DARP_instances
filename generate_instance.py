...
PATH = Path.cwd()
INSTANCE_PATH = PATH.parent / "Instances"
...
def generate_instance(city: str, instance_config_path: Path, results_path: Path):
    ...
    generate_demand(map_nodes, config_instance, nnp, crs_metric)

    # Generate vehicles
    # desired_vehicle_count = 24
    # generate_vehicles(map_nodes, config_instance, nnp, desired_vehicle_count)
...
if __name__ == "__main__":
    ...
    for start, durations in start_durations.items():
        for duration in durations:
            for delay in delays:
                instance_config_path = INSTANCE_PATH / f'{city}/instances/' / f'start_{start}' / f'duration_{duration}' / f'max_delay_{delay}' / 'config.yaml'
                results_path = RESULTS_PATH / f'{city}' / f'start_{start}' / f'duration_{duration}' / f'max_delay_{delay}' / method / 'config.yaml'
