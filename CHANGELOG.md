...
- changed (breaking, unreleased format only): seating unified on the slot-type model — vehicles declare alternative seating `configurations` (slot type → count mappings over `standard`, `wheelchair`, `electric_wheelchair`, `stroller`; `capacity` remains as sugar for one all-standard layout), passengers are per-slot-type counts (`passengers_*` request columns, previously `demand_*`), and the onboard load must fit at least one configuration at every stop (the fitting layout may change mid-operation — configurations model shared physical spots). The recently added `wheelchair_slots` vehicle field is removed (use a configuration); `WHEELCHAIR_CAPACITY` verdicts are now reported as `CAPACITY`; `child_seats` stays a separate equipment count. Legacy `Slot_Type` request columns and `slots`/enum-keyed `configurations` vehicle fields are translated on load.
- changed: `Demand` class renamed to `Passengers` (defaults to one standard passenger); per-request hard constraints (`required_vehicle_id`, `exclusive`, `required_equipment`) moved from `Request` into `RequestConstraints`; the legacy integer equipment mechanism and `EquipmentType` enum are removed

- added: unified extended instance format (all fields optional, legacy instances unchanged): `max_delay` (canonical name of `max_travel_time_delay`), boarding-anchored `max_travel_delay` (absolute/relative), per-request constraint override columns (inherit / override / disabled), composite demand (seats, wheelchair slots, child seats), named vehicle equipment, exclusive rides, per-request service times, required arrival times with a symmetric earliness bound, walking distance limits, per-vehicle driver rules (total/continuous drive time, minimum pause) and operation windows checked per stop, per-vehicle `return_to_depot`, and a distance matrix (`dist_filepath`) — documented in the README per-file reference sections (instance config, requests, vehicles)
