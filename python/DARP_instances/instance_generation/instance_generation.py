...
import DARP_instances.instance_generation.map
import DARP_instances.instance_generation.demand_generation
import DARP_instances.instance_generation.vehicles
...
def generate_instance(config_filepath: str):
    ...
    map_nodes, map_edges = DARP_instances.instance_generation.map.get_map(config)

    crs_metric = config['map']['SRID_plane']
    nodes = map_nodes.to_crs(f'epsg:{crs_metric}')
    nearest_node_provider = DARP_instances.instance_generation.map.NearestNodeProvider(nodes)

    # generate dm
    generate_dm(config, map_nodes, map_edges)

    # Generate trips
    trips = DARP_instances.instance_generation.demand_generation.generate_demand(
    ...
    DARP_instances.instance_generation.vehicles.generate_vehicles(
