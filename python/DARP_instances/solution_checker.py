...
import DARP_instances.amodsim_benchmark

from typing import Tuple, Set
from DARP_instances.cordeau_benchmark import load as load_cordeau
from DARP_instances.instance import Action, ActionType, DARPInstance, TravelTimeProvider, Request
from DARP_instances.solution import VehiclePlan, Solution
...
def load_data(solution_file_path: str) -> Tuple[DARPInstance, Solution]:
    ...
    solution = DARP_instances.inout.load_solution(solution_file_path, instance)
...
def load_instance(instance_path: str) -> Tuple[DARPInstance, TravelTimeProvider]:
    if instance_path.endswith('yaml'):
        instance = DARP_instances.amodsim_benchmark.read_instance(instance_path)
        travel_time_provider = instance.travel_time_provider
    else:
        instance = load_cordeau(instance_path)
        travel_time_provider = DARP_instances.instance.EuclideanTravelTimeProvider(60)
