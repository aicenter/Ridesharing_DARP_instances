...
import DARP_instances.instance
...
def load_results_from_folder(folder_path: str, skip_used_connections: bool = False) \
    ...
    for file in os.listdir(directory):
        ...
        if "solution.json" in filename:
            solutions.append(DARP_instances.inout.load_json(filepath))
        elif "performance.json" in filename:
            performances.append(DARP_instances.inout.load_json(filepath))
...
def load_instance_series(series_file_path: str) -> Tuple[Dict[str, Dict[str, List[dict]]],Dict[str,Dict[str, List[dict]]]]:
    solutions = {}
    performances = {}
...
def load_aggregate_stats_for_result(path: Path) -> Optional[Dict]:
    result, performance = load_results_from_folder(str(path))
    if type(result) is list:
        if len(result) == 0:
            return None
    data, _ = get_processed_results(result, performance, return_as_dict=True)

    config_path = path / 'config.yaml'
    exp_config = DARP_instances.experiments.load_experiment_config(str(config_path))
    data['method'] = exp_config['method']

    instance_config_path = path / exp_config['instance']
    instance_config = DARP_instances.instance.load_instance_config(str(instance_config_path))
